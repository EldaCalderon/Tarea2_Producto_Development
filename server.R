...
shinyServer(function(input, output) {
  ...
  output$contenido_archivo_2 <- DT::renderDataTable({
    archivo_carga_2() %>% DT::datatable(filter = 'bottom')
  })
  ...
  output$tabla4 <- renderDataTable({
    mtcars %>% datatable(selection = 'single') # solo se puede seleccionar una fila
  })
  ...
  output$tabla5 <- renderDataTable({
    mtcars %>% datatable() # se puede seleccionar multiples fila
  })
